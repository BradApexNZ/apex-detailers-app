...
const privateCall = name => async payload => {
  try {
    ...
    throw new Error(error?.message || "Apex cloud services are temporarily unavailable.");
  }
};
const calendarGatewayCall = apexCloudEnabled
  ? privateCall("listGoogleCalendarsV6")
  : async payload => {
...
const readLegacyGoogleCalendarStatus = apexCloudEnabled
...
export const saveGoogleCalendarSelection = async payload => {
  ...
  const writable = new Set(
    calendars
      .filter(row => row.writable || ["owner", "writer"].includes(row.accessRole))
      .map(row => String(row.id || ""))
      .filter(Boolean)
  );
  const requested = Array.isArray(payload?.selectedCalendarIds)
    ? payload.selectedCalendarIds.map(String).filter(id => allowed.has(id))
    : [];
  ...
  if (!primaryCalendarId)
    primaryCalendarId =
      ...
      calendars.find(row => row.primary && writable.has(String(row.id)))?.id ||
      [...writable][0] ||
      "";
  ...
  await setDoc(
    doc(db, "settings", "googleCalendar"),
    { selectedCalendarIds, primaryCalendarId, updatedAt: serverTimestamp() },
    { merge: true }
  );
  statusPromise = null;
