...
const customerName = customer =>
  clean(customer.businessName || [customer.firstName, customer.lastName].filter(Boolean).join(" ") || customer.customerName);
const customerKey = customer => [normal(customer.email), phoneKey(customer.phone), normal(customerName(customer))].join("|");
const jobCompositeKey = job =>
  [normal(job.bookingDate), normal(job.bookingTime), normal(job.customerName), normal(job.vehicle || job.rego)].join("|");
const jobKeys = job => {
...
function App() {
  ...
  const [user, setUser] = useState(null),
    ...
    [customerFile, setCustomerFile] = useState(null),
    [jobFile, setJobFile] = useState(null);
  ...
  async function login(event) {
    ...
    try {
      ...
      setMessage("Login failed. Check your Firebase email and password.");
    }
    setBusy(false);
  }
  ...
  async function backup() {
    ...
    try {
      ...
      for (const name of names) {
        ...
        data.collections[name] = snapshot.docs.map(item => ({ id: item.id, ...serialise(item.data()) }));
      }
  ...
  async function exportCustomers() {
    ...
    try {
      ...
      download(
        `apex-customers-${new Date().toISOString().slice(0, 10)}.json`,
        snapshot.docs.map(item => ({ id: item.id, ...serialise(item.data()) }))
      );
  ...
  async function importCustomers() {
    ...
    setBusy(true);
    setMessage("Checking customer file…");
    try {
      ...
      for (const raw of rows) {
        const row = {
          ...
          area: clean(raw.area),
          preferredContact: clean(raw.preferredContact) || "text",
          customerType: clean(raw.customerType) || "standard",
      ...
      setMessage(error.message || "Customer import failed.");
    }
    setBusy(false);
  }
  ...
  async function importJobs() {
    ...
    setBusy(true);
    setMessage("Checking booking history and matching customers…");
    try {
      ...
      for (const raw of rows) {
        ...
        if (!row.customerName || !row.bookingDate) {
          incomplete++;
          continue;
        }
        ...
        const matchedCustomerId =
          customerByEmail.get(normal(row.email)) ||
          customerByPhone.get(phoneKey(row.phone)) ||
          customerByName.get(normal(row.customerName)) ||
          "";
      ...
      await flush();
      setMessage(
        `Booking import complete: ${added} added, ${duplicates} duplicates skipped, ${incomplete} incomplete skipped, ${linked} linked to existing customers.`
      );
    } catch (error) {
      setMessage(error.message || "Booking import failed.");
    }
    setBusy(false);
  }
    ...
    return (
      <main className="page">
        ...
        <form onSubmit={login}>
          ...
          <h1>Apex HQ Data Tools</h1>
          <p>Export backups and safely import customer or booking-history JSON files.</p>
          <label>
            Email
    ...
    <main className="page">
      ...
      <header>
        ...
        <div>
          ...
          <button className="secondary" onClick={signOutAndClearCache}>
            Sign out
          </button>
        </div>
      </header>
      <section className="grid">
        <article>
          <h2>Full business backup</h2>
          <p>Downloads customers, jobs, vouchers, booking requests and inquiries into one dated JSON file.</p>
        ...
        <article>
          ...
          <p>Downloads only the customer database for a lighter contact backup.</p>
          <button disabled={busy} onClick={exportCustomers}>
            Download customers
          </button>
        </article>
        <article>
          ...
          <p>
            Accepts a customer array, a <code>customers</code> array, or an Apex backup. Existing email, phone and name matches are skipped.
          </p>
          <input type="file" accept="application/json,.json" onChange={event => setCustomerFile(event.target.files?.[0] || null)} />
          <button disabled={busy || !customerFile} onClick={importCustomers}>
            Import customer file
          </button>
        </article>
        <article>
          <h2>Import booking history</h2>
          <p>
            Imports past and future jobs from an Apex booking JSON file. Calendar IDs and job details prevent duplicates; existing customers
            are linked automatically where possible.
          </p>
          <input type="file" accept="application/json,.json" onChange={event => setJobFile(event.target.files?.[0] || null)} />
          <button disabled={busy || !jobFile} onClick={importJobs}>
            Import booking file
          </button>
        </article>
      </section>
      {message && <div className="notice fixed">{message}</div>}
      <footer>Download a full backup before every large import. Unknown prices remain $0 until you confirm them.</footer>
    </main>
  );
}
