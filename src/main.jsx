...
const pipelineStatuses = ["Lead", "Quote Sent", "Approved", "Booked", "In Progress", "Completed", "Paid"];
...
const navItems = [["dashboard", "Home", "⌂"], ["customers", "Clients", "◌"], ["jobs", "Jobs", "◆"], ["bookings", "Diary", "◷"], ["media", "Photos", "▣"]];
...
function LoginScreen({ loading, error, onLogin }) {
  ...
  return <div className="appShell loginShell">
    <main className="appMain loginMain">
      <section className="heroCard loginHero" aria-labelledby="login-title">
        <BrandMark />
        <div>
          <span className="eyebrow">Apex HQ V3</span>
          <h2 id="login-title">Detailing command centre.</h2>
          <p>Private access for Apex Detailers quotes, bookings, job notes, customer records and photo proof.</p>
        </div>
      </section>
      <form className="card loginCard" onSubmit={submit}>
        ...
        <p className="muted">Secure owner login for Apex Detailers.</p>
      </form>
    </main>
  </div>;
}
...
function App() {
  ...
  function openJobModal({ mode = "quote", customer = null, job = null } = {}) {
    ...
    setJobForm({ ...emptyJob, mode, status: mode === "past" ? "Completed" : mode === "booking" ? "Booked" : "Quote Sent", serviceDate: mode === "past" ? todayISO() : "", customerId: customer?.id || "", customerName: customer ? displayCustomerName(customer) : "", phone: customer?.phone || "", email: customer?.email || "", address: customer?.address || "", area: customer?.area || "", preferredContact: customer?.preferredContact || "text", customerType: customer?.customerType || "standard" });
    setModal("addJob");
  }
  ...
  if (!authReady) return <div className="appShell"><main className="appMain"><section className="card loadingCard"><BrandMark /><h2>Loading Apex HQ...</h2></section></main></div>;
  if (!isBrad) return <LoginScreen loading={loginLoading} error={loginError} onLogin={handleLogin} />;

  return <div className="appShell">
    <a className="skipLink" href="#main-content">Skip to main content</a>
    <header className="appHeader">
      <div className="brandHeader" aria-label="Apex Detailers">
        <BrandMark />
      ...
      <button type="button" className="logoutButton" onClick={handleLogout} aria-label="Log out of Apex Detailers">
    ...
    <main className="appMain" id="main-content">
      <section className="heroCard heroV3" aria-labelledby="home-hero-title">
        <div className="heroCopy">
          <span className="eyebrow">Apex HQ V3</span>
          <h2 id="home-hero-title">Your detailing command centre.</h2>
          <p>Quotes, bookings, client details, job photos and paid work — all polished into one fast mobile workspace for Apex Detailers.</p>
          <div className="heroPills" aria-label="Apex HQ highlights">
            <span>Fast quotes</span>
            <span>Clean diary</span>
            <span>Photo proof</span>
          </div>
        </div>
        <div className="heroConsole" aria-label="Apex HQ summary">
          <span>Today</span>
          <strong>{bookedJobs.filter(job => job.bookingDate === todayISO()).length} booked</strong>
          <small>{jobs.length} total job records · {photoCount} photos saved</small>
      ...
      {tab === "bookings" && <JobsTab title="Upcoming Diary" jobs={bookedJobs.slice().sort((a, b) => String(a.bookingDate).localeCompare(String(b.bookingDate)))} openJobModal={openJobModal} updateJobStatus={updateJobStatus} deleteJob={deleteJob} />}
    ...
    <nav className="bottomNav" aria-label="Primary navigation">{navItems.map(([id, label, icon]) => <button key={id} type="button" onClick={() => setTab(id)} className={tab === id ? "active" : ""} aria-current={tab === id ? "page" : undefined}><span className="navIcon" aria-hidden="true">{icon}</span><span>{label}</span></button>)}</nav>
...
function Dashboard({ customers, jobs, bookedJobs, completedJobs, quotedRevenue, paidRevenue, photoCount, openCustomerModal, openJobModal }) {
  ...
  const todayJobs = bookedJobs.filter(job => job.bookingDate === today);
  const pendingQuotes = jobs.filter(job => ["Lead", "Quote Requested", "Quote Sent", "Approved"].includes(job.status || "Quote Sent")).length;
  const activeJobs = jobs.filter(job => ["Booked", "In Progress"].includes(job.status || "")).length;
  const pipelineCounts = pipelineStatuses.map(status => ({ status, count: jobs.filter(job => (job.status || "Quote Sent") === status).length }));
  const latestJob = jobs[0];
  const nextBooking = bookedJobs.slice().sort((a, b) => `${a.bookingDate || ""} ${a.bookingTime || ""}`.localeCompare(`${b.bookingDate || ""} ${b.bookingTime || ""}`))[0];

  return <section className="dashboardPage" aria-label="Apex dashboard">
    <div className="commandGrid">
      <article className="commandCard mainCommand">
        <span className="eyebrow">Today's run sheet</span>
        <h2>{todayJobs.length ? `${todayJobs.length} job${todayJobs.length === 1 ? "" : "s"} booked today` : "Ready for the next booking."}</h2>
        <p>{nextBooking ? `Next up: ${nextBooking.customerName || "Customer"} · ${nextBooking.vehicle || "Vehicle"} · ${nextBooking.bookingDate}${nextBooking.bookingTime ? " at " + nextBooking.bookingTime : ""}.` : latestJob ? `Latest: ${latestJob.customerName || "Customer"} · ${latestJob.vehicle || "Vehicle"} · ${money(latestJob.total)}.` : "Start by adding a quote, booking, or completed job with photos."}</p>
    ...
    <div className="statGrid homeStats" aria-label="Apex stats">
    ...
    <div className="quickGrid actionGrid" aria-label="Quick actions">
      <button type="button" className="actionCard" onClick={() => openCustomerModal()}><span>Client</span><strong>＋ Add details</strong><em>Name, phone, address and notes</em></button>
      <button type="button" className="actionCard" onClick={() => openJobModal({ mode: "quote" })}><span>Quote</span><strong>＋ Price a job fast</strong><em>Package, condition and add-ons</em></button>
      <button type="button" className="actionCard" onClick={() => openJobModal({ mode: "past" })}><span>Proof</span><strong>＋ Save job photos</strong><em>Before, after and stain records</em></button>
    </div>

    <article className="card pipelineCard">
      <div className="cardHeader"><div><h3>Job pipeline</h3><p className="muted">A clear snapshot of quotes, bookings, work in progress and paid jobs.</p></div></div>
      <div className="pipelineRail">{pipelineCounts.map(item => <div className="pipelineStep" key={item.status}><strong>{item.count}</strong><span>{item.status}</span></div>)}</div>
    </article>

    <article className="card recentCard">
      <div className="cardHeader"><div><h3>Recent work</h3><p className="muted">Latest quotes, bookings and completed details.</p></div></div>
      {jobs.slice(0, 5).map(job => <CompactJob key={job.id} job={job} />)}
      {!jobs.length && <div className="emptyMini"><strong>No jobs yet.</strong><span>Add a quote or past job to start testing Apex HQ V3.</span></div>}
    </article>
  </section>;
}

function CustomersTab({ customers, customerJobs, openCustomerModal, openJobModal, deleteCustomer }) {
  return <section><div className="sectionTitle"><div><h2>Clients</h2><p>Save details, notes, vehicles and repeat-work history without making the app feel like boring database software.</p></div></div><div className="quickGrid"><button type="button" onClick={() => openCustomerModal()}>＋ Add Client</button><button type="button" className="secondary" onClick={() => openJobModal({ mode: "past" })}>＋ Add Past Job + Photos</button></div>{customers.map(customer => { const rows = customerJobs(customer.id); const total = rows.reduce((sum, job) => sum + Number(job.total || 0), 0); const photos = rows.reduce((sum, job) => sum + (job.photos || []).length, 0); return <article className="card customerCard" key={customer.id}><div className="cardHeader"><div><h3>{displayCustomerName(customer)}</h3><p className="muted">{customer.phone || "No phone"}{customer.email ? ` • ${customer.email}` : ""}{customer.area ? ` • ${customer.area}` : ""}</p></div><span className="badge">{rows.length} job{rows.length === 1 ? "" : "s"}</span></div><div className="detailList">{customer.address && <span>Address: {customer.address}</span>}{customer.preferredContact && <span>Preferred contact: {customer.preferredContact}</span>}{customer.lastVehicle && <span>Last vehicle: {customer.lastVehicle}</span>}{customer.notes && <span>Notes: {customer.notes}</span>}<span>Total quoted: {money(total)}</span><span>Saved photos from jobs: {photos}</span></div><div className="buttonRow"><button type="button" className="secondary" onClick={() => openJobModal({ mode: "quote", customer })}>＋ Quote</button><button type="button" className="secondary" onClick={() => openJobModal({ mode: "past", customer })}>＋ Past Job + Photos</button><button type="button" className="secondary" onClick={() => openCustomerModal(customer)}>Edit Details</button><button type="button" className="dangerGhost" onClick={() => deleteCustomer(customer.id)}>Delete</button></div>{rows.length > 0 && <div className="detailList">{rows.slice(0, 3).map(job => <span key={job.id}>{job.vehicle} • {job.packageName} • {money(job.total)} • {(job.photos || []).length} photo(s)</span>)}</div>}</article>; })}{!customers.length && <div className="emptyState"><h3>Add your first client</h3><p>Save name, phone, email, address and notes, then attach jobs and photos.</p><button type="button" onClick={() => openCustomerModal()}>＋ Add Client</button><button type="button" className="secondary" onClick={() => openJobModal({ mode: "past" })}>＋ Add Past Job + Photos</button></div>}</section>;
}

function JobsTab({ title, jobs, openJobModal, updateJobStatus, deleteJob }) {
  return <section><div className="sectionTitle"><div><h2>{title}</h2><p>Quotes, bookings, completed jobs and paid work all live here.</p></div><button type="button" className="smallButton" onClick={() => openJobModal({ mode: "past" })}>＋ Job</button></div>{jobs.map(job => <JobCard key={job.id} job={job} openJobModal={openJobModal} updateJobStatus={updateJobStatus} deleteJob={deleteJob} />)}{!jobs.length && <div className="emptyState"><h3>No jobs here yet</h3><p>Add a quote, booking, or completed job with photos.</p><button type="button" onClick={() => openJobModal({ mode: "past" })}>＋ Add Past Job + Photos</button></div>}</section>;
}
...
function CustomerForm({ form, updateForm, saving, saveCustomer }) {
  return <section><h2>{form.id ? "Edit Client" : "Add Client"}</h2><p className="muted">Add name, phone, email, address, area, contact preference and notes. Photos go on the client's jobs.</p><div className="formGrid"><Input label="First Name" value={form.firstName} onChange={v => updateForm("firstName", v)} /><Input label="Last Name" value={form.lastName} onChange={v => updateForm("lastName", v)} /></div><Input label="Business Name / Fleet Name" value={form.businessName} onChange={v => updateForm("businessName", v)} /><Input label="Phone" value={form.phone} onChange={v => updateForm("phone", v)} inputMode="tel" /><Input label="Email" value={form.email} onChange={v => updateForm("email", v)} type="email" /><Input label="Street / Address" value={form.address} onChange={v => updateForm("address", v)} /><Input label="Area / Suburb" value={form.area} onChange={v => updateForm("area", v)} placeholder="Napier, Hastings, Poraiti..." /><div className="formGrid"><Select label="Preferred Contact" value={form.preferredContact} onChange={v => updateForm("preferredContact", v)} options={contactMethods} /><Select label="Customer Type" value={form.customerType} onChange={v => updateForm("customerType", v)} options={customerTypes} /></div><Textarea label="Client Notes" value={form.notes} onChange={v => updateForm("notes", v)} placeholder="Access notes, regular customer, dog hair, fleet account, anything useful..." /><button type="button" disabled={saving} onClick={saveCustomer}>{saving ? "Saving..." : form.id ? "Save Client Changes" : "Save Client"}</button></section>;
}
...
function CompactJob({ job }) { return <div className="compactJob"><div><strong>{job.customerName || "Customer"}</strong><span>{job.vehicle || "Vehicle"} • {job.status || "Quote Sent"}</span></div><b>{money(job.total)}</b></div>; }
...
function BrandMark() { return <div className="brandLogo" aria-hidden="true"><span className="logoWing logoWingLeft" /><span className="logoCore"><b>A</b><em>APEX</em></span><span className="logoWing logoWingRight" /></div>; }
