...
const navItems = [["dashboard", "Home", "⌂"], ["customers", "Customers", "◌"], ["jobs", "Jobs", "◆"], ["bookings", "Bookings", "◷"], ["media", "Photos", "▣"]];
    ...
    <main className="appMain loginMain">
      <section className="heroCard loginHero">
        ...
        <div>
          ...
          <p>Private access for Apex Detailers jobs, quotes, bookings, customer details and photo records.</p>
        </div>
      </section>
  ...
  return <div className="appShell">
    <header className="appHeader">
      <div className="brandHeader">
        <BrandMark />
        <div className="wordmark"><h1>Apex Detailers</h1><p>Apex HQ V3</p></div>
      </div>
      <button type="button" className="logoutButton" onClick={handleLogout} aria-label="Log out of Apex Detailers">Logout</button>
    </header>
    <main className="appMain">
      <section className="heroCard heroV3">
        <div className="heroCopy">
          ...
          <p>Built for fast quotes, clean bookings, proper customer history and sharp job photos while you are out working in Hawke's Bay.</p>
          <div className="heroPills" aria-label="Apex HQ highlights"><span>Fast quote flow</span><span>Photo records</span><span>Customer ready</span></div>
        </div>
  ...
  return <section className="dashboardPage" aria-label="Apex dashboard">
    <div className="commandGrid">
      <article className="commandCard mainCommand">
        ...
        <p>{latestJob ? `Latest: ${latestJob.customerName || "Customer"} - ${latestJob.vehicle || "Vehicle"} at ${money(latestJob.total)}.` : "Start by adding a quote, booking, or completed job with photos."}</p>
    ...
    <div className="quickGrid actionGrid">
      <button type="button" className="actionCard" onClick={() => openCustomerModal()}><span>Customer</span><strong>＋ Add client details</strong><em>Name, phone, address and notes</em></button>
      ...
      <button type="button" className="actionCard" onClick={() => openJobModal({ mode: "past" })}><span>Photos</span><strong>＋ Save past work</strong><em>Before, after and stain records</em></button>
    </div>

    <article className="card pipelineCard">
      <div className="cardHeader"><div><h3>Job pipeline</h3><p className="muted">Clear status snapshots for quotes, bookings and paid work.</p></div></div>
...
function ActionSheet({ close, openCustomerModal, openJobModal }) { return <div className="sheetBackdrop" onClick={close}><div className="actionSheet" role="dialog" aria-modal="true" aria-label="Quick add menu" onClick={e => e.stopPropagation()}><div className="sheetHandle" /><h2>Quick add</h2><p className="muted">Big buttons for fast iPhone entry.</p><button type="button" onClick={() => openCustomerModal()}>＋ Add Customer</button><button type="button" onClick={() => openJobModal({ mode: "quote" })}>＋ New Quote / Booking</button><button type="button" onClick={() => openJobModal({ mode: "past" })}>＋ Completed Past Job + Photos</button><button type="button" className="secondary" onClick={close}>Cancel</button></div></div>; }
...
function BrandMark() { return <div className="brandLogo" aria-hidden="true"><span>AD</span></div>; }
