...
const owners = () => OWNER_UIDS.value().split(",").map(v => v.trim()).filter(Boolean);
...
export const saveGoogleCalendarSelection = onCall({ region: REGION, secrets: SECRETS }, async request => {
  ...
  if (!integration.exists || !integration.data().refreshToken) throw new HttpsError("failed-precondition", "Reconnect Google Calendar first.");
  ...
  const requested = Array.isArray(request.data?.selectedCalendarIds) ? request.data.selectedCalendarIds.map(clean).filter(id => allowed.has(id)) : [];
  ...
  if (!primaryCalendarId) primaryCalendarId = selectedCalendarIds.find(id => writable.has(id)) || rows.find(r => r.primary && writable.has(r.id))?.id || [...writable][0] || "";
  ...
  await db.doc("settings/googleCalendar").set({ selectedCalendarIds, primaryCalendarId, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
